export interface Track {
  name: string;
  lat: number;
  lon: number;
}

export const tracks: Track[] = [
  { name: "Autodrom Most", lat: 50.51884205271267, lon: 13.604885534091473},
  { name: "Teststrecke Freital", lat: 51.00307024827095, lon: 13.635912669543},
  { name: "RedBull Ring", lat: 47.221320912541664, lon: 14.762632556193969},
  { name: "Hungaroring", lat: 47.58035314980727, lon: 19.249563651526643},
  { name: "VDE e-Race", lat: 53.09673113139393, lon: 14.325061409266697},
  { name: "TT Circuit Assen", lat: 53.09673113139393, lon: 14.325061409266697},
  { name: "FS Switzerland", lat: 46.50204377450582, lon: 8.299866965030827},
  { name: "ZalaZone Proving ground", lat: 46.89503895667208, lon: 16.843557985060553},
  { name: "Hockenheimring", lat: 49.33056050531237, lon: 8.565524837436797 }
];